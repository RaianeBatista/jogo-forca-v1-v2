# Projeto 1 - Desenvolvimento de Game em Linguagem Pyhton - Ver~soa 1
# Import
import random
...
def game():
    ...
    palavra = random.choice(palavras) # choice = escolher
    ...
    while chances > 0:
        ...
        #Verificação de entrada de mais de 1 letra
        if len(tentativa) != 1:
        ...
        if tentativa.isdigit():
            print("Números não são permitidos! Digite apenas uma letra.")
            continue

        # Condicional
        if tentativa in letras_tentativas:
            print("Você já tentou esssa letra. Escolha outra!")
            continue
